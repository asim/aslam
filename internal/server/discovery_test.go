...
func TestKnowledgeDiscoveryContract(t *testing.T) {
	...
	data, _ := readDiscoveryDocument("/openapi.json")
	...
	data, _ = readDiscoveryDocument("/.well-known/api-catalog")
...
func readDiscoveryDocument(path string) ([]byte, error) {
	mux := http.NewServeMux()
	discovery.RegisterRoutes(mux)
	w := httptest.NewRecorder()
